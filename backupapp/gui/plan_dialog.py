...
class PlanDialog(QDialog):
    def __init__(self, app: AppConfig, plan: BackupPlan | None = None, parent=None):
        ...
        self.setWindowTitle(_("编辑备份计划") if plan else _("新增备份计划"))
        ...
        self._retention_unit.addItem(_("份"), "count")
        self._retention_unit.addItem(_("天"), "days")
        ...
        self._password.setPlaceholderText(_("zip 使用 AES 加密；tar.gz 不支持密码"))
        self._exclude = QPlainTextEdit("\n".join(plan.exclude) if plan else "")
        self._exclude.setPlaceholderText(_("每行一个排除规则（通配符），如 cache、*.log"))
        self._backup_mode = QComboBox()
        self._backup_mode.addItem(_("原样复制"), "copy")
        self._backup_mode.addItem(_("替换为链接（数据存备份目录）"), "link")
        self._backup_mode.setCurrentIndex(0 if (plan is None or plan.backup_mode == "copy") else 1)
        self._restore_mode = QComboBox()
        self._restore_mode.addItem(_("原样复制回源路径"), "copy")
        self._restore_mode.addItem(_("替换为链接（不拷贝）"), "link")
        self._restore_mode.setCurrentIndex(0 if (plan is None or plan.restore_mode == "copy") else 1)
        self._link_type = QComboBox()
        self._link_type.addItem(_("junction（免管理员，推荐）"), "junction")
        ...
        self._cmd_timeout.setSuffix(_(" 秒"))
        timeout_row = QHBoxLayout()
        timeout_row.addWidget(self._cmd_timeout)

        # 计划任务排期：与全局一致，或自定义
        self._schedule_mode = QComboBox()
        self._schedule_mode.addItem(_("与全局一致"), "global")
        self._schedule_mode.addItem(_("自定义"), "custom")
        self._schedule_mode.setCurrentIndex(
            0 if (plan is None or plan.schedule_mode == "global") else 1)
        self._schedule_freq = QComboBox()
        for val, label in (("daily", _("每天")), ("weekly", _("每周")), ("days", _("每N天")),
                           ("hourly", _("每N小时")), ("minutely", _("每N分钟")),
                           ("atLogon", _("登录时"))):
        ...
        for d in range(1, 8):
            self._schedule_day.addItem(
                _("周{day}").format(day="一二三四五六日"[d - 1]), d)
        ...
        form.addRow(_("源路径"), self._sources)
        form.addRow("", preset_row)
        form.addRow(_("目的路径"), dest_row)
        form.addRow(_("保留"), retention_row)
        form.addRow("", self._keep_monthly)
        form.addRow("", self._keep_yearly)
        form.addRow("", self._compress)
        form.addRow(_("压缩格式"), self._format)
        form.addRow(_("压缩密码"), self._password)
        form.addRow(_("排除规则"), self._exclude)
        ...
        form.addRow(_("时间"), self._schedule_time)
        form.addRow(_("星期"), self._schedule_day)
    ...
    def _browse_dest(self):
        p = QFileDialog.getExistingDirectory(
            self, _("选择目的目录"), self._destination.text() or "")
        if p:
    ...
    def _accept(self):
        ...
        if not self._sources.paths():
            QMessageBox.warning(self, _("输入不完整"), _("至少需要一个源路径"))
            return
        self.accept()
