...
- **命令钩子**：每个备份计划可配置"备份前/备份后"命令或脚本（如停止/启动服务），可设超时；前置失败中止备份，后置失败仅记日志
...
- **多语言**：简体中文 / English 双语界面（GUI 与 CLI），语言可在工具栏切换或跟随系统自动检测；文档（README / DEVELOPMENT / CHANGELOG）提供英文版
...
安装后开始菜单出现 **BackupApp** 快捷方式（GUI），命令行可用 `backupapp`（即 `backupapp-cli.exe`）。配置与数据保存在 `backupapp\data`，已配置 persist，`scoop update` 升级不丢失。注册的计划任务指向 `current` 链接，升级后不会失效。
